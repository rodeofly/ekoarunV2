      ...
      if ui.draggable.is "ul"
        ...
        switch op3.type
          when "multiplication"
            ...
            match2 = /([-+]?\d+(?:\/\d+)?)[\.+](.*)/g.exec( str2 )
            match3 = /([-+]?\d+(?:\/\d+)?)[\.+](.*)/g.exec( str3 )
            console.log match2, match3
            if ( match2? and match3? and (match2.length is 3) and (match3.length is 3 ) )
...
generate_equation = (unknown=2, factor_length=2, depth=2, min = -10, max = 10) ->
  ...
  create_equation_membre = ->  
    ...
    for j in [1..factor_length]
      ...
      for j in [1..unknown]
        m = new Monome()
        m.randomize((alphabet.shuffle())[j],1).insert op.id
      $( operateur.id ).append( $( "#fake" ) ) 
    $( "#fake" ).remove()
  ...
  create_equation_membre()  
  $( "#equation_#{id}" ).append( "<ul id='fake' data-symbol='*'></ul>" ).trigger( "click" )
  create_equation_membre()  
  megateuf()
